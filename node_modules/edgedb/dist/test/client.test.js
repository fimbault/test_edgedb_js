    ...
    try {
        ...
        for (const [types, values] of intCases) {
            for (const type of types) {
                res = await con.queryOne(`select (<${type}>$0 + <${type}>$1,);`, values);
        ...
        const ldt = new index_node_1.LocalDateTime(2012, 5, 30, 14, 11, 33, 123);
        res = await con.queryOne(`select <cal::local_datetime>$0`, [ldt]);
        expect(res instanceof index_node_1.LocalDateTime).toBeTruthy();
        expect(res.getHours()).toBe(14);
        expect(res.toISOString()).toBe("2012-06-30T14:11:33.123");
...
test("fetch: cal::local_date", async () => {
    ...
    try {
      ...
      `);
        expect(res instanceof datetime_1.LocalDate).toBeTruthy();
...
test("fetch: duration fuzz", async () => {
    ...
    const durs = [
        new datetime_1.Duration(),
        new datetime_1.Duration(1),
        new datetime_1.Duration(-1),
        new datetime_1.Duration(1),
        new datetime_1.Duration(-1),
        new datetime_1.Duration(-752043.296),
        new datetime_1.Duration(3542924),
        new datetime_1.Duration(86400000),
        new datetime_1.Duration(-86400000),
    ];
    for (let _i = 0; _i < 5000; _i++) {
        durs.push(new datetime_1.Duration(randint(-500, 500) * 86400 + randint(-1000, 1000)));
    }
    const con = await testbase_1.asyncConnect();
    try {
...
test("fetch: tuple", async () => {
    ...
    try {
        res = await con.query("select ()");
        expect(res).toEqual([[]]);
        expect(index_node_1._introspect(res)).toEqual({ kind: "set" });
        expect(index_node_1._introspect(res[0])).toEqual({ kind: "tuple" });
        ...
        expect(insp === "Tuple [ 1, 'abc' ]" || insp === "Tuple(2) [ 1, 'abc' ]").toBeTruthy();
    }
    finally {
        await con.close();
    }
...
test("fetch: object", async () => {
    ...
    try {
      ...
      limit 1
    `);
        expect(index_node_1._introspect(res.params[0])).toEqual({
            kind: "object",
            fields: [
        ...
        expect(util.inspect(res.params[0])).toBe("Object [ kind := 'POSITIONAL', num := 0, @foo := 42 ]");
        expect(res.params.length).toBe(2);
        expect(res.params[0].id instanceof index_node_1.UUID).toBeTruthy();
        expect(res.params[0].__tid__ instanceof index_node_1.UUID).toBeTruthy();
        expect(res.params[1].__tid__).toEqual(res.params[0].__tid__);
        expect(res.id instanceof index_node_1.UUID).toBeTruthy();
        expect(res.__tid__ instanceof index_node_1.UUID).toBeTruthy();
...
test("fetch: set of arrays", async () => {
    ...
    try {
      ...
      limit 1
    `);
        expect(index_node_1._introspect(res)).toEqual({
            kind: "object",
            fields: [
                { name: "__tid__", implicit: true, linkprop: false },
        ...
        res = res.sets;
        expect(index_node_1._introspect(res)).toEqual({ kind: "set" });
        expect(index_node_1._introspect(res[0])).toEqual({ kind: "array" });
        expect(res).toEqual([[1, 2], [1]]);
        expect(res.length).toBe(2);
        expect(res instanceof index_node_1.Set).toBeTruthy();
        expect(res instanceof Array).toBeTruthy();
        expect(res[1] instanceof index_node_1.Set).toBeFalsy();
...
test("fetch: uuid", async () => {
    ...
    try {
        ...
        expect(res instanceof index_node_1.UUID).toBeTruthy();
        expect(res.buffer.length).toBe(16);
        res = await con.queryOne("SELECT <uuid>'759637d8-6635-11e9-b9d4-098002d459d5'");
        expect(res instanceof index_node_1.UUID).toBeTruthy();
        expect(res.buffer.length).toBe(16);
...
test("fetch: namedtuple", async () => {
    ...
    try {
        ...
        expect(util.inspect(t0)).toBe("NamedTuple [ a := 'aaa', b := true, c := 123 ]");
    }
    finally {
        await con.close();
    }
...
test("queryOne: arrays", async () => {
    ...
    try {
        ...
        expect(index_node_1._introspect(res)).toEqual({ kind: "array" });
...
test("fetch: long strings", async () => {
    ...
    try {
        ...
        expect(res.length).toEqual(1000000);
        await con
            ...
            .catch((e) => {
            expect(e.toString()).toMatch(/query result is too big: buffer overflow/);
        });
    }
    finally {
        await con.close();
...
test("execute", async () => {
    ...
    try {
        await con
            ...
            expect(e instanceof index_node_1.DivisionByZeroError).toBeTruthy();
            expect(e instanceof index_node_1.MissingRequiredError).toBeFalsy();
            expect(e instanceof index_node_1.EdgeDBError).toBeTruthy();
            expect(e.code).toBe(83951617);
        });
        await con.execute("start transaction isolation serializable");
        try {
            const isolation = await con.queryOne("select sys::get_transaction_isolation()");
