...
const con_utils_1 = require("../src/con_utils");
const testbase_1 = require("./testbase");
function env_wrap(env, func) {
    const old_env = {};
...
function runConnectionTest({ env = {}, opts, result, error, }) {
    if (error) {
        expect(() => {
            env_wrap(env, con_utils_1.parseConnectArguments.bind(null, opts));
        }).toThrow(error);
    }
    else {
        env_wrap(env, () => {
            const args = con_utils_1.parseConnectArguments(opts);
...
test("parseConnectArguments", () => {
    const TESTS = [
        ...
        },
        {
            ...
            error: 'dsn "pq:///dbname?host=/unix_sock/test&user=spam" ' +
...
test("connect: timeout", async () => {
    let con;
    try {
        con = await testbase_1.asyncConnect({ timeout: 1 });
